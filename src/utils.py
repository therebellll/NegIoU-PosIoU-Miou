...
def calc_iou_tensor_diag(boxes1, boxes2):
    """
    Return intersection-over-union (Jaccard index) of boxes.
    ...
    #  When the shapes do not match,
    #  the shape of the returned output tensor follows the broadcasting rules
    lt = torch.max(boxes1[:, :2], boxes2[:, :2])  # left-top [N,2]
    rb = torch.min(boxes1[:, 2:], boxes2[:, 2:])  # right-bottom [N,2]
...
def calc_c_tensor(boxes1, boxes2):
    """
    Return c
    ...
    #  When the shapes do not match,
    #  the shape of the returned output tensor follows the broadcasting rules
    lt = torch.min(boxes1[:, :2], boxes2[:, :2])  # left-top [N,M,2]
    rb = torch.max(boxes1[:, 2:], boxes2[:, 2:])  # right-bottom [N,M,2]




    wh = (rb - lt).clamp(min=0)  # [N,M,2]
    c = torch.sqrt( torch.pow(wh[:, 0],2) + torch.pow(wh[:, 1],2))  # [N,M]
...
class Encoder(object):
    ...
    def encode(self, bboxes_in, labels_in, criteria=0.6):
        """
        encode:
            input  : bboxes_in (Tensor nboxes x 4), labels_in (Tensor nboxes)
            output : bboxes_out (Tensor 8732 x 4), labels_out (Tensor 8732)
        ...
        # filter IoU > 0.7
        # 寻找与bbox_in iou大于0.7的default box,对应论文中Matching strategy的第二条(这里包括了第一条匹配到的信息)
        masks = best_dbox_ious > criteria
        # [8732,]
        labels_out = torch.zeros(self.nboxes, dtype=torch.int64)
        labels_out[masks] = labels_in[best_dbox_idx[masks]]
        bboxes_out = self.dboxes.clone()
        # 将default box匹配到正样本的地方设置成对应GT box信息
...
class DefaultBoxes(object):
    def __init__(self, fig_size, feat_size, steps, scales, aspect_ratios, scale_xy=0.1, scale_wh=0.2):
        ...
        for idx, sfeat in enumerate(self.feat_size):
            all_sizes = []
            for scale, ratio in zip(scales[idx], aspect_ratios[idx]):
                sk = scale / fig_size[0]  # scale转为相对值[0-1]
                all_sizes.append((sk, sk / ratio))

            # 计算当前特征层对应原图上的所有default box
            for w, h in all_sizes:
                for i in range(sfeat[1]):
                    for j in range(sfeat[0]):
                        cx = (j + 0.5) / sfeat[0]
                        cy = (i + 0.5) / sfeat[1]
...
def dboxes300_coco():
    figsize = [640, 360]  # 输入网络的图像大小
    feat_size = [[80, 45], [40, 23]]  # 每个预测层的feature map尺寸
    steps = [8, 16, 32]  # 每个特征层上的一个cell在原图上的跨度
    scales = [[57], [87, 136]]  # 每个特征层上预测的default box的scale
    aspect_ratios = [[1.19], [1.22, 1.01]]  # 每个预测特征层上预测的default box的ratios
