...
class NightDataSet(Dataset):
    ...
    def __init__(self, night_root, transforms,  train_set='train.txt'):
        ...
        with open(txt_list) as read:
            self.xml_list = [os.path.join(self.annotations_root, "sample" + line.strip() + ".xml")
                             for line in read.readlines()]

        with open(txt_list) as read:
            self.ids = [line.strip() for line in read.readlines()]
    ...
    def __getitem__(self, idx):
        image, target, h, w = self.pull_item(idx)
        return image, target
    ...
    def pull_item(self, idx):
        ...
        return image, target, data_height, data_width
